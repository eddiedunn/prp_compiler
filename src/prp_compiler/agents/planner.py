...
class PlannerAgent(BaseAgent):
    ...
    def plan(self, 
             ...
             schemas_manifest: List[ManifestItem]) -> ExecutionPlan:
        """
        Generates an execution plan by calling the LLM.
        """
        # Convert manifests to JSON strings for the prompt
        tools_json = json.dumps([item.model_dump() for item in tools_manifest], indent=2)
        knowledge_json = json.dumps([item.model_dump() for item in knowledge_manifest], indent=2)
        schemas_json = json.dumps([item.model_dump() for item in schemas_manifest], indent=2)
