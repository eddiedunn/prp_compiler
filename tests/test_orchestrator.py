...
def test_orchestrator_dynamic_action(mock_import_module, mock_loader, mock_knowledge_store):
    ...
    def mock_planning_loop(*args, **kwargs):
        action = Action(tool_name="retrieve_knowledge", arguments={"query": "test query"})
        yield ReActStep(thought=Thought(reasoning="test", criticism="test", next_action=action))
        # Immediately finish
        finish_action = Action(tool_name="finish", arguments={"schema_choice": "my_schema", "pattern_references": []})
        yield ReActStep(thought=Thought(reasoning="done", criticism="none", next_action=finish_action))
