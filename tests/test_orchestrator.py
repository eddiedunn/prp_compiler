...
def sample_manifests(temp_capabilities):
    ...
    tools = [
        ManifestItem(name='test_tool', description='A test tool', file_path=str(temp_capabilities["tool"]))
    ]
    knowledge = [
        ManifestItem(name='test_knowledge', description='A knowledge doc', file_path=str(temp_capabilities["knowledge"]))
    ]
    schemas = [
        ManifestItem(name='test_schema', description='A schema', file_path=str(temp_capabilities["schema"]))
    ]
...
def test_assemble_context_schema_not_found(orchestrator):
    ...
    plan = ExecutionPlan(
        tool_plan=[],
        knowledge_plan=[],
        schema_choice="non_existent_schema"
    )
    with pytest.raises(ValueError, match="Schema 'non_existent_schema' not found in manifest."):
...
def test_resolve_dynamic_content_file_not_found(orchestrator):
    ...
    result = orchestrator._resolve_dynamic_content(raw_context)
    assert result == "[ERROR: File not found at '/tmp/this_file_should_not_exist_12345.txt']"
